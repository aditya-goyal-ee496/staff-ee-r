...
(`docs/rules/task-execution.md`): spec → contract test → simplest impl → `make format/test/lint` →
...
- **Orchestrator** — the `/orchestrate` slash command, run on **Opus**. It discovers workflows
  (`docs/orchestration/workflows/*.js`), reads each one's `meta.orchestrator` manifest, gathers
...
- **Workflows** — JS Workflow-engine scripts that own *all* the logic for one job. Each declares a
  ...
  is **`build-feature`** (`docs/orchestration/workflows/build-feature.js`): spec → decompose → tests
  (eval-first) → implement → quality → verify → architecture/ADR → progress report → finalize.
- **Model tiers** (`docs/orchestration/model-usage.md`): Opus orchestrates; Sonnet plans, specs,
...
- **Autonomy modes** — `gate` (default; pause at every gate, honouring `task-execution.md`),
...
- **Per-run JSON ledger** — the orchestrator mints a dated, unique `runId` (Bash, since the sandbox
  ...
  `index.jsonl`. Shapes in `docs/orchestration/ledger-schema.md`.

Because `.claude/` is git-ignored, the **tracked source of truth lives under `docs/`** (workflow
scripts under `docs/orchestration/workflows/`, command definitions under `docs/commands/`); slash
commands are installed locally into `.claude/commands/` via `make sync-claude`, and runtime logs stay
in `.claude/` uncommitted.

## Consequences

- Adding a new workflow is a self-contained change: drop a `docs/orchestration/workflows/<name>.js`
